...
// Function to swap two nodes (changes the links, not the data)
    ...
    do {
        ...
        while (ptr1->next != lptr && ptr1->next != *head) {
            if (ptr1->data > ptr1->next->data) {
                ...
                swapped = 1;
                // Swap the pointers for ptr1 and ptr1->next since they are swapped
                Node* temp = ptr1;
                ptr1 = ptr1->prev; // Move ptr1 back to the swapped position
                ptr1->next = temp; // Fix the next pointer
            }
            ptr1 = ptr1->next;
        }
        lptr = ptr1;
